...
def get_object_by_name(object_name):
    """Get object by name or raise error if not found"""
    obj = bpy.data.objects.get(object_name)
    if not obj:
        raise ValueError(f"Object '{object_name}' not found, create first")
    return obj
...
def asymmetric_jitter(x, y, intensity=RANDOMNESS_FACTOR):
...
def deform_terrain(terrain_obj, terrain_mode="mountain"):
    ...
    for vertex in terrain_obj.data.vertices:
        ...
        if terrain_mode == "mountain":
            base_height = abs(math.sin(FREQUENCY * x + PHASE_X) * math.cos(FREQUENCY * y + PHASE_Y)
                            + MIX_WEIGHT * math.sin(MIX_FREQUENCY * FREQUENCY * y + PHASE_MIX) + PHASE_Z)
            z = HEIGHT_SCALE * base_height**POWER_VALUE * math.exp(-DECAY_RATE * radius**2)
        else:
            z = 0.0
...
# Main execution
def main():
    terrain = get_object_by_name(TERRAIN_OBJECT_NAME)
    # Generate base terrain
    deform_terrain(terrain, TERRAIN_MODE)
    # Apply disturbances
    apply_smart_jitter(terrain)
    smooth_height_by_slope(terrain)
